...
export default function HouseLoanDashboard() {
  ...
  // Set your password here (in a real app, this would be more secure)
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100">
        <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-2xl shadow-xl">
          <div className="text-center">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">🏠 House Loan Dashboard</h1>
          ...
          <div className="text-center text-xs text-gray-500 mt-6">
            <p>🔒 Your data is protected with password authentication</p>
            <p className="mt-2">💡 <strong>Tip:</strong> Upload your 'house-loan-data.json' file to load your data</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white shadow-xl rounded-2xl mt-10">
      ...
      <div className="flex justify-between items-start mb-8">
        <div className="text-center flex-1">
          <h1 className="text-3xl font-bold text-blue-800 mb-2">🏠 House Loan Dashboard</h1>
          {loanData && (
            <div className="text-sm text-gray-600 space-y-1">
              ...
              <p><strong>Original Amount:</strong> €{loanData.loanDetails.amount.toLocaleString()} | <strong>Interest Rate:</strong> {loanData.loanDetails.interestRate}% | <strong>Term:</strong> {loanData.loanDetails.termYears} years</p>
          ...
          {loanStatus && (
            <div className="mb-8 bg-gradient-to-r from-purple-50 to-blue-50 p-6 rounded-xl border border-purple-200">
              <h2 className="text-xl font-semibold mb-4 text-purple-800">
                ...
                {loanStatus.loanPaidOff && <span className="ml-2 text-green-600">(LOAN PAID OFF! 🎉)</span>}
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <p className="text-lg font-bold text-green-600">€{loanStatus.totalPaid.toLocaleString()}</p>
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <p className="text-lg font-bold text-blue-600">€{loanStatus.principalPaid.toLocaleString()}</p>
                  <p className="text-xs text-gray-500">Of loan amount</p>
                </div>
                <div className="bg-white p-4 rounded-lg shadow-sm">
                  <h3 className="font-semibold text-orange-800 text-sm">Interest Paid</h3>
                  <p className="text-lg font-bold text-orange-600">€{loanStatus.interestPaid.toLocaleString()}</p>
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <p className="text-lg font-bold text-red-600">€{loanStatus.remainingBalance.toLocaleString()}</p>
                  <p className="text-xs text-gray-500">{loanStatus.loanPaidOff ? 'Paid off early!' : `${loanStatus.monthsRemaining} months left`}</p>
                </div>
              </div>

              {!loanStatus.loanPaidOff && schedule.length > 0 && (
                <div className="space-y-6">
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <h3 className="font-semibold text-gray-800">Loan Principal Progress</h3>
                      <span className="text-sm text-gray-600 font-medium">
                        {((loanStatus.principalPaid / loanData.loanDetails.amount) * 100).toFixed(1)}% Paid Off
                      </span>
                    </div>
                    <div className="flex rounded-lg overflow-hidden h-8 bg-gray-200">
                      <div 
                        className="bg-blue-500 flex items-center justify-center text-white text-xs font-medium"
                        style={{ width: `${(loanStatus.principalPaid / loanData.loanDetails.amount) * 100}%` }}
                        title={`Principal Paid: €${loanStatus.principalPaid.toLocaleString()}`}
                      >
                        {((loanStatus.principalPaid / loanData.loanDetails.amount) * 100) > 15 ? 'Principal Paid' : ''}
                      </div>
                      <div 
                        className="bg-gray-400 flex items-center justify-center text-white text-xs font-medium"
                        style={{ width: `${(loanStatus.remainingBalance / loanData.loanDetails.amount) * 100}%` }}
                        title={`Remaining Balance: €${loanStatus.remainingBalance.toLocaleString()}`}
                      >
                        {((loanStatus.remainingBalance / loanData.loanDetails.amount) * 100) > 15 ? 'Remaining Balance' : ''}
                      </div>
                    </div>
                    <div className="flex justify-between text-xs text-gray-600 mt-2">
                      <span>€0</span>
                      <span>€{loanData.loanDetails.amount.toLocaleString()} (Original Loan)</span>
                    </div>
                  </div>

                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <h3 className="font-semibold text-gray-800">Total Payment Progress (Principal + Interest)</h3>
                      <span className="text-sm text-gray-600 font-medium">
                        {((loanStatus.totalPaid / (loanData.loanDetails.amount + schedule[schedule.length - 1].totalInterest)) * 100).toFixed(1)}% Complete
                      </span>
                    </div>
                    <div className="flex rounded-lg overflow-hidden h-8 bg-gray-200">
                      <div 
                        className="bg-blue-500 flex items-center justify-center text-white text-xs font-medium"
                        style={{ 
                          width: `${(loanStatus.principalPaid / (loanData.loanDetails.amount + schedule[schedule.length - 1].totalInterest)) * 100}%` 
                        }}
                        title={`Principal Paid: €${loanStatus.principalPaid.toLocaleString()}`}
                      >
                        {((loanStatus.principalPaid / (loanData.loanDetails.amount + schedule[schedule.length - 1].totalInterest)) * 100) > 12 ? 'Principal' : ''}
                      </div>
                      <div 
                        className="bg-orange-500 flex items-center justify-center text-white text-xs font-medium"
                        style={{ 
                          width: `${(loanStatus.interestPaid / (loanData.loanDetails.amount + schedule[schedule.length - 1].totalInterest)) * 100}%` 
                        }}
                        title={`Interest Paid: €${loanStatus.interestPaid.toLocaleString()}`}
                      >
                        {((loanStatus.interestPaid / (loanData.loanDetails.amount + schedule[schedule.length - 1].totalInterest)) * 100) > 12 ? 'Interest Paid' : ''}
                      </div>
                      <div 
                        className="bg-red-400 flex items-center justify-center text-white text-xs font-medium"
                        style={{ 
                          width: `${((loanStatus.remainingBalance + (schedule[schedule.length - 1].totalInterest - loanStatus.interestPaid)) / (loanData.loanDetails.amount + schedule[schedule.length - 1].totalInterest)) * 100}%` 
                        }}
                        title={`Remaining Payments: €${(loanStatus.remainingBalance + (schedule[schedule.length - 1].totalInterest - loanStatus.interestPaid)).toLocaleString()}`}
                      >
                        {(((loanStatus.remainingBalance + (schedule[schedule.length - 1].totalInterest - loanStatus.interestPaid)) / (loanData.loanDetails.amount + schedule[schedule.length - 1].totalInterest)) * 100) > 15 ? 'Remaining Payments' : ''}
                      </div>
                    </div>
                    <div className="flex justify-between text-xs text-gray-600 mt-2">
                      <span>€0</span>
                      <span>€{(loanData.loanDetails.amount + schedule[schedule.length - 1].totalInterest).toLocaleString()} (Total Cost)</span>
                    </div>
                  </div>
                </div>
              )}
          ...
          {schedule.length > 0 && comparison.length > 0 && (
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <p className="text-sm">Total interest: <span className="font-bold">€{schedule[schedule.length - 1]?.totalInterest.toLocaleString()}</span></p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="text-sm">Total interest: <span className="font-bold">€{comparison[comparison.length - 1]?.totalInterest.toLocaleString()}</span></p>
              </div>
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-sm">Interest saved: <span className="font-bold">€{(comparison[comparison.length - 1]?.totalInterest - schedule[schedule.length - 1]?.totalInterest).toLocaleString()}</span></p>
              </div>
            </div>
          )}
            ...
            <div className="mt-8">
              ...
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={chartData}>
                  ...
                  />
                  <Legend />
                  <Line type="monotone" dataKey="totalPrincipal" stroke="#22c55e" name="Total Principal Paid (With Extras)" strokeWidth={3} dot={false} />
                  <Line type="monotone" dataKey="totalInterest" stroke="#f59e0b" name="Total Interest Paid (With Extras)" strokeWidth={3} dot={false} />
                  <Line type="monotone" dataKey="remainingBalance" stroke="#3b82f6" name="Remaining Balance (With Extras)" strokeWidth={3} dot={false} />
                  <Line type="monotone" dataKey="totalPrincipalNoExtras" stroke="#86efac" name="Total Principal Paid (No Extras)" strokeDasharray="8 8" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="totalInterestNoExtras" stroke="#fbbf24" name="Total Interest Paid (No Extras)" strokeDasharray="8 8" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="remainingBalanceNoExtras" stroke="#93c5fd" name="Remaining Balance (No Extras)" strokeDasharray="8 8" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="border border-gray-200 px-4 py-2 font-semibold">Payment (€)</th>
                      <th className="border border-gray-200 px-4 py-2 font-semibold">Principal (€)</th>
                      <th className="border border-gray-200 px-4 py-2 font-semibold">Interest (€)</th>
                      <th className="border border-gray-200 px-4 py-2 font-semibold">Additional Payment (€)</th>
                      <th className="border border-gray-200 px-4 py-2 font-semibold">Remaining Balance (€)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(showFullTable ? schedule : schedule.slice(0, 12)).map((row) => (
                      <tr key={row.month} className={`hover:bg-gray-50 ${row.month === currentMonth ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''}`}>
                        <td className="border border-gray-200 px-4 py-2 text-center font-medium">
                          {row.month}
                          {row.month === currentMonth && <span className="ml-2 text-blue-600 text-xs">← Current</span>}
                        </td>
                        ...
                        <td className="border border-gray-200 px-4 py-2 text-right">
                          {row.oneTimePayment > 0 ? (
                            <span className="text-green-600 font-semibold">€{row.oneTimePayment.toLocaleString()}</span>
                          ) : (
                            '-'
                          )}
                        </td>
